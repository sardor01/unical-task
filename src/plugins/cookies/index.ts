import type { CookiesConfig, VueCookies } from './interfaces';
...
class VueCookiesManager implements VueCookies {
  private readonly current_default_config: CookiesConfig;
  ...
  config(config: CookiesConfig): void {
    let propertyName: keyof typeof this.current_default_config;
    for (propertyName in this.current_default_config) {
      // prettier-ignore
      // @ts-expect-error: Unreachable code error
      this.current_default_config[propertyName] = config[propertyName] || defaultConfig[propertyName]
    }
  }
  ...
  get(keyName: string): string {
    let value = decodeURIComponent(
      document.cookie.replace(
        new RegExp(
          `(?:(?:^|.*;)\\s*${encodeURIComponent(keyName).replace(/[-.+*]/g, '\\$&')}\\s*\\=\\s*([^;]*).*$)|^.*$`,
        ),
        '$1',
      ),
    );
  ...
  set(
    ...
    if (!keyName) {
      ...
      throw new Error(
        `Cookie name illegality. Cannot be set to ["expires","max-age","path","domain","secure","SameSite"]\t current key name: ${keyName}`,
      );
    }
    // support json object
    ...
    if (expireTimes && expireTimes !== 0) {
      switch (expireTimes.constructor) {
        case Number:
          if (expireTimes === Number.POSITIVE_INFINITY || expireTimes === -1)
        ...
        case String:
          if (/^(?:\d+(y|m|d|h|min|s))$/i.test(<string>expireTimes)) {
            ...
            switch ((<string>expireTimes).replace(/^(?:\d+)(y|m|d|h|min|s)$/i, '$1').toLowerCase()) {
              // Frequency sorting
              case 'm':
    ...
    document.cookie = `${encodeURIComponent(keyName)}=${encodeURIComponent(value)}${_expires}${
  ...
  remove(keyName: string, path?: string, domain?: string): boolean {
    ...
    document.cookie = `${encodeURIComponent(keyName)}=; expires=Thu, 01 Jan 1970 00:00:00 GMT${
      domain ? `; domain=${domain}` : this.current_default_config.domain ? this.current_default_config.domain : ''
    }${
      path ? `; path=${path}` : this.current_default_config.path ? this.current_default_config.path : '; path=/'
    }; SameSite=Lax`;
  ...
  isKey(keyName: string): boolean {
    return new RegExp(`(?:^|;\\s*)${encodeURIComponent(keyName).replace(/[-.+*]/g, '\\$&')}\\s*\\=`).test(
      document.cookie,
    );
  }
...
const globalCookiesConfig = (options: CookiesConfig) => {
...
const useCookies = () => {
  ...
  return { cookies: GLOBAL_COOKIES_MANAGER };
};
