...
const pages = computed(() => {
  ...
  if (pageCount.value <= pageRange.value) {
    ...
    const setBreakView = (index: number) => {
      ...
      items[index] = { ...items[index], ...breakView };
    };
    ...
    for (let i = selectedRangeLow; i <= selectedRangeHigh && i <= pageCount.value - 1; i++) setPageItem(i);

    if (selectedRangeLow > marginPages.value) setBreakView(selectedRangeLow - 1);

    if (selectedRangeHigh + 1 < pageCount.value - marginPages.value) setBreakView(selectedRangeHigh + 1);
